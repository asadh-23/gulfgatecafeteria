    ...
    <header className="h-16 bg-white border-b border-gray-200 fixed top-0 left-[240px] right-0 z-20 flex items-center justify-between px-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div ref={panelRef} className="relative">
          ...
          {/* Dropdown Panel */}
          {open && (
            <div className="absolute right-0 top-12 w-80 bg-white rounded-2xl shadow-2xl z-50 overflow-hidden border border-gray-100">
              ...
              <div className="flex items-center justify-between px-4 pt-4 pb-2">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-gray-900 font-bold text-sm">Notification</h3>
                    <p className="text-xs text-gray-400">{pendingOrders.length} pending order{pendingOrders.length !== 1 ? 's' : ''}</p>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-2 px-4 pb-2 border-b border-gray-100">
                <button onClick={() => setTab('pending')}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                    tab === 'pending' ? 'bg-green-600 text-white' : 'text-gray-500 hover:bg-gray-100'}`}>
                  Pending
                  {pendingOrders.length > 0 && (
                    <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold ${tab === 'pending' ? 'bg-white text-green-600' : 'bg-gray-200 text-gray-600'}`}>
              ...
              <div className="max-h-80 overflow-y-auto divide-y divide-gray-50">
                {pendingOrders.length === 0 ? (
                  ...
                  pendingOrders.map((order) => (
                    <Link key={order._id} href="/orders" onClick={() => setOpen(false)}
                      className={`flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors ${!seenIds.has(order._id) ? 'bg-orange-50/40' : ''}`}>
                      <div className="w-9 h-9 rounded-full bg-orange-50 flex items-center justify-center flex-shrink-0 text-base">🛒</div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          <p className="text-xs font-bold text-orange-600">New Order</p>
                          <span className="text-[10px] text-gray-400 whitespace-nowrap">{formatTime(order.createdAt)}</span>
                        </div>
                        <p className="text-xs font-semibold text-gray-700 mt-0.5">#{order.orderNumber}</p>
                        <p className="text-xs text-gray-500 mt-0.5">{order.customerName} · AED {order.totalAmount.toFixed(2)}</p>
                        {!seenIds.has(order._id) && (
                          <span className="mt-1.5 inline-block px-2 py-0.5 bg-orange-100 text-orange-700 text-[10px] font-bold rounded-full">New</span>
                        )}
                      </div>
                      {!seenIds.has(order._id) && <span className="w-2 h-2 bg-orange-500 rounded-full flex-shrink-0 mt-1" />}
                    </Link>
                  ))
                )}
              </div>
              ...
              <div className="px-4 py-3 border-t border-gray-100 bg-gray-50">
                <Link href="/orders" onClick={() => setOpen(false)}
                  className="flex items-center justify-center gap-1 w-full py-2 bg-green-600 hover:bg-green-700 text-white text-xs font-bold rounded-xl transition-colors">
                  Manage All Orders
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="w-9 h-9 rounded-full bg-green-600 flex items-center justify-center text-white font-bold text-sm">G</div>
        </div>
